...
namespace Cyotek.AzureContainerEcho
{
  public class EchoScheduledTask : ScheduledTask
  {
    ...
    protected override void ExecuteJob()
    {
      ...
      // download
      pendingRemote = this.GetMissingOrChangedRemoteBlobs(remoteItems, options.LocalPath, options.CheckForNewFilesOnly);
    ...
    private IList<ICloudBlob> GetMissingOrChangedRemoteBlobs(IEnumerable<KeyValuePair<string, ICloudBlob>> remoteItems, string localPath, bool checkForNewFilesOnly)
    {
      ...
      foreach (KeyValuePair<string, ICloudBlob> pair in remoteItems)
      {
        ...
        remoteHash = !checkForNewFilesOnly && !string.IsNullOrEmpty(fileBlob.Properties.ContentMD5) ? this.GetBlobHash(fileBlob) : Guid.Empty;

        if (!File.Exists(localFileName) || !checkForNewFilesOnly && (remoteHash == Guid.Empty || remoteHash != this.GetMd5HashFromFile(localFileName)))
